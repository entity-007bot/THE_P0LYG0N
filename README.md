...
For the product, ecosystem, architecture, scaling, revenue, and compliance narrative, see docs/POLYGON_BRIEF.md.
...
4. AI matching – powered by ARIA ai – ranks open gigs from the local job database.
...
POLYGON is built as a shared operating layer for these partners:
...
· AI job matching using local scoring and optional Transformers.js embeddings (branded as ARIA ai).
...
POLYGON Control Panel (admin only – no public link):
